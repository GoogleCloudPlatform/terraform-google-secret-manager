...
resource "google_secret_manager_regional_secret" "secrets" {
  project = var.project_id
  ...
  dynamic "customer_managed_encryption" {
    for_each = lookup(each.value, "customer_managed_encryption", null) == null ? [] : [each.value.customer_managed_encryption]
    content {
...
resource "google_secret_manager_regional_secret_version" "secret-version" {
  for_each    = { for secret in var.secrets : secret.name => secret if secret.create_version }
  secret      = google_secret_manager_regional_secret.secrets[each.value.name].id
...
resource "google_secret_manager_regional_secret_iam_binding" "binding" {
  ...
  secret_id = google_secret_manager_regional_secret.secrets[each.value.name].id
